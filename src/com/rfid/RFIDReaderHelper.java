...
/**
 * The core class to operate RFID reader.You can send command to RFID reader via
 * the methods. And you can receive the return data via register
 * {@link #Observer} or if you want show the return data in UI ,you can register
 * {@link #HandlerObserver}.
 */
public class RFIDReaderHelper extends ReaderHelper {
	 ...
	 * 
	 * @param btReadId      Reader Address(0xFF Public Address)
	 * @param btWorkAntenna Ant ID(0x00:Ant 1, 0x01:Ant 2, 0x02:Ant 3, 0x03:Ant
	 *                      4,0x04:Ant 5, 0x05:Ant 6, 0x06:Ant 7, 0x07:Ant 8)
	 * @return Succeeded :0, Failed:-1
	 */
	...
	/**
	 * Set output power(Set the same output power of all antennas). <br>
	 ...
	 * 
	 * @param btReadId      Reader Address(0xFF Public Address)
	 * @param btOutputPower RF output power, range from 0 to 33(0x00 - 0x21), the
	 *                      unit is dBm.
	 * @return Succeeded :0, Failed:-1
	 */
	 ...
	 * @param btPower4 Output power of antenna 4, range from 0 to 33(0x00 - 0x21),
	 *                 the unit is dBm.
	 * @param btPower5 Output power of antenna 5, range from 0 to 33(0x00 - 0x21),
	 *                 the unit is dBm.
	 * @param btPower6 Output power of antenna 6, range from 0 to 33(0x00 - 0x21),
	 *                 the unit is dBm.
	 * @param btPower7 Output power of antenna 7, range from 0 to 33(0x00 - 0x21),
	 *                 the unit is dBm.
	 * @param btPower8 Output power of antenna 8, range from 0 to 33(0x00 - 0x21),
	 *                 the unit is dBm.
	 * @return Succeeded :0, Failed:-1
	 */
	public final int setOutputPower(byte btReadId, byte btPower1, byte btPower2, byte btPower3, byte btPower4,
	 ...
	 * @param btEndRegion   End frequency of the spectrum,Setup the range of the RF
	 *                      output spectrum. The rules are: 1,Start frequency and
	 *                      end frequency should be in the range of the specified
	 *                      regulation. 2,Start frequency should be equal or lower
	 *                      than end frequency. 3, End frequency equals start
	 *                      frequency means use single frequency point.
	 * @return Succeeded :0, Failed:-1
	 */
	 ...
	 * 
	 * @param btReadId          Reader Address(0xFF Public Address)
	 * @param btFreqInterval    Frequency space, frequency space = FreqSpace x
	 *                          10KHz.
	 * @param btChannelQuantity Frequency Quantity, this quantity includes the start
	 *                          frequency, if set this byte to 1, it means use start
	 *                          frequency as the single carrier frequency . This
	 *                          byte should be larger than 0.
	 * @param nStartFreq        Start Frequency, the unit is KHz. Set the start
	 *                          frequency with hex format, for example, 915000KHz =
	 *                          0D F6 38 KHz.
	 * @return Succeeded :0, Failed:-1
	 */
	 ...
	 * 
	 * @param btReadId Reader Address(0xFF Public Address)
	 * @param btMode   Buzzer behavior(0x00:Quiet, 0x01:Beep after every inventory
	 *                 round if tag(s) identified, 0x02:Beep after every tag has
	 *                 identified.)
	 * @return Succeeded :0, Failed:-1
	 */
	 ...
	 * 
	 * @param btReadId         Reader Address(0xFF Public Address)
	 * @param btDetectorStatus status(0x00:close detector of antenna connection,
	 *                         other values:sensitivity of antenna connection
	 *                         detector(return loss of port),unit dB. The higher the
	 *                         value, the greater the impedance matching
	...
	/**
	 * Set temporary output power. <br>
	 * The output power value will Not be saved to the internal flash memory so that
	 * the output power will be restored from the internal flash memory after
	 * restart or power off.
	 * 
	 * @param btReadId  Reader Address(0xFF Public Address)
	 * @param btRfPower RF output power, range from 20-33(0x14 - 0x21), the unit is
	 *                  dBm.
	 * @return Succeeded :0, Failed:-1
	 */
	 ...
	 * 
	 * @param btReadId  Reader Address(0xFF Public Address)
	 * @param btProfile Communication rate(0xD0:Tari 25uS,FM0 40KHz, 0xD1:Tari
	 *                  25uS,Miller 4 250KHz, 0xD2:Tari 25uS,Miller 4 300KHz,
	 ...
	 * 
	 * @param btReadId    Reader Address(0xFF Public Address)
	 * @param btFrequency FreqParameter, system will measure the return loss of
	 *                    current antenna port at the desired frequency.
	 * @return Succeeded :0, Failed:-1
	 */
	...
	/**
	 * Inventory Tag. <br>
	 * When reader gets this command, the inventory for EPC GEN2 tags starts, tag
	 * data will be stored in the internal buffer. <br>
	 * Attention: <br>
	 ...
	 * 
	 * @param btReadId Reader Address(0xFF Public Address)
	 * @param btRepeat Repeat time of inventory round. When Repeat = 255, The
	 *                 inventory duration is minimized. For example, if the RF field
	 *                 only has one or two tags, the inventory duration could be
	 *                 only 30-50 mS, this function provides a possibility for fast
	 ...
	 * Read Tag. <br>
	 * Attention: <br>
	 * If two tags have the same EPC, but different read data, then these two tags
	 * are considered different tags.
	 * 
	 * @param btReadId      Reader Address(0xFF Public Address)
	 * @param btMemBank     Tag memory bank(0x00:RESERVED, 0x01:EPC, 0x02:TID,
	 *                      0x03:USER)
	 * @param btWordAdd     Read start address,please see the tag's spec for more
	 *                      information.
	 * @param btWordCnt     Read data length,Data length in WORD(16bits) unit.
	 ...
	 * @param btMemBank     Tag memory bank(0x00:RESERVED, 0x01:EPC, 0x02:TID,
	 *                      0x03:USER)
	 * @param btWordAdd     Write start address,WORD(16 bits). When write EPC area,
	 *                      notice that EPC starts from address 02, the first two 2
	 *                      words are for PC+CRC.
	 * @param btWordCnt     WORD(16 bits), please see the tag's spec for more
	 *                      information.
	 ...
	 * @param btReadId      Reader Address(0xFF Public Address)
	 * @param btAryPassWord Access password, 4 bytes.
	 * @param btMemBank     Tag memory bank(0x01:User Memory, 0x02:TID Memory,
	 *                      0x03:EPC Memory, 0x04:Access Password, 0x05:Kill
	 *                      Password)
	 * @param btLockType    Lock operation type(0x00:Open, 0x01:Lock, 0x02:Permanent
	 *                      open, 0x03:Permanent lock)
	 * @return Succeeded :0, Failed:-1
	 */
	...
	/**
	 * Inventory Tag(Read Time Mode). <br>
	 * Attention: <br>
	 * The hardware has a dual CPU architecture, main CPU is responsible for tag
	 * inventory, and assistant CPU is responsible for data management. Inventory
	 * and data transfer are parallel and simultaneous. So the data transfer via
	 * serial port doesn't affect the efficiency of reader.
	 * 
	 * @param btReadId Reader Address(0xFF Public Address)
	 * @param btRepeat Repeat time of inventory round. When Repeat = 255, The
	 *                 inventory duration is minimized. For example, if the RF field
	...
	/**
	 * Fast Switch Antenna Mode. <br>
	 * Attention: <br>
	 * The hardware has a dual CPU architecture, main CPU is responsible for tag
	 * inventory, and assistant CPU is responsible for data management. Inventory
	 * and data transfer are parallel and simultaneous. So the data transfer via
	 * serial port doesn't affect the efficiency of reader. <br>
	 * In massive tag applications, please use cmd_real_time_inventory command which
	 * is more effective for large tag quantity applications.
	 * 
	 * @param btReadId   Reader Address(0xFF Public Address)
	 * @param btA        First working ant (00 - 03). If set this byte above 03
	 *                   means ignore it.
	 * @param btStayA    Inventory round for an antenna.
	 * @param btB        Second working ant (00 - 03). If set this byte above 03
	 *                   means ignore it.
	 * @param btStayB    Inventory round for an antenna.
	 * @param btC        Third working ant (00 - 03). If set this byte above 03
	 *                   means ignore it.
	 * @param btStayC    Inventory round for an antenna.
	 * @param btD        Fourth working ant (00 - 03). If set this byte above 03
	 *                   means ignore it.
	 * @param btStayD    Inventory round for an antenna.
	 * @param btInterval Rest time between switching antennas. During the cause of
	 *                   rest, RF output will be cancelled, thus power consumption
	 *                   and heat generation are both reduced.
	...
	/**
	 * Fast Switch Antenna Mode(8 Antennas). <br>
	 * Attention: <br>
	 * The hardware has a dual CPU architecture, main CPU is responsible for tag
	 * inventory, and assistant CPU is responsible for data management. Inventory
	 * and data transfer are parallel and simultaneous. So the data transfer via
	 ...
	 * 
	 * @param btReadId   Reader Address(0xFF Public Address)
	 * @param btA        First working ant (00 - 07). If set this byte above 07
	 *                   means ignore it.
	 * @param btStayA    Inventory round for an antenna.
	 * @param btB        Second working ant (00 - 07). If set this byte above 07
	 *                   means ignore it.
	 * @param btStayB    Inventory round for an antenna.
	 * @param btC        Third working ant (00 - 07). If set this byte above 07
	 *                   means ignore it.
	 * @param btStayC    Inventory round for an antenna.
	 * @param btD        Fourth working ant (00 - 07). If set this byte above 07
	 *                   means ignore it.
	 * @param btStayD    Inventory round for an antenna.
	 * @param btE        First working ant (00 - 07). If set this byte above 07
	 *                   means ignore it.
	 ...
	 */
	public final int fastSwitchAntInventory(byte btReadId, byte btA, byte btStayA, byte btB, byte btStayB, byte btC,
			byte btStayC, byte btD, byte btStayD, byte btE, byte btStayE, byte btF, byte btStayF, byte btG,
			byte btStayG, byte btH, byte btStayH, byte btInterval, byte btRepeat) {
	 ...
	 * changed to altered PC + EPC + EPC's CRC + TID. <br>
	 * If error occurred during identifying TID, only the original data (PC + EPC)
	 * will be sent. <br>
	 * If you don't need this function, please turn it off, otherwise there will be
	 * unnecessary time consumption. <br>
	...
	/**
	 * Get tag data and keep buffer. <br>
	 * Attention: <br>
	 * The data in the buffer won't be lost after execution of this command. <br>
	 ...
	 * @param btMaskNo   The mask No include(0x00,0x01,0x02,0x03,0x04),you set
	 *                   different mask via the mask No.
	 * @param btTarget   Set the inventory way(s0,s1,s2 or s3),you must use you set
	 *                   target to inventory the tag.
	 * @param btAction   The match tag or not Action,you can see the detail of this
	 *                   command.
	 ...
	 * @return Succeeded :0, Failed:-1
	 */
	public final int setTagMask(byte btReadId, byte btMaskNo, byte btTarget, byte btAction, byte btMembank,
			byte btStartAdd, byte btMaskLen, byte[] maskValue) {
	 ...
	 * 
	 * @param btReadId the reader address.
	 * @param btMaskNo The mask No include(0x00,0x01,0x02,0x03,0x04),you clear any
	 *                 mask via the mask No.
	 * @return Succeeded :0, Failed:-1
	 */
