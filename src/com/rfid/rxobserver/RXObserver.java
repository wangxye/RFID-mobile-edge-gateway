...
public class RXObserver implements Observer {
	...
	private void processGetFrequencyRegion(MessageTran msgTran) {
		...
		if (btAryData.length == 0x03) {
			...
			m_curReaderSetting.nUserDefineStartFrequency = (btAryData[3] & 0xFF) * 256 * 256
					+ (btAryData[4] & 0xFF) * 256 + (btAryData[5] & 0xFF);
			refreshSetting(m_curReaderSetting);
			onExeCMDStatus(btCmd, ERROR.SUCCESS);
			return;
		} else if (btAryData.length == 0x01) {
	...
	private void processInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x09) {
			...
			end.mTagCount = (btAryData[1] & 0xFF) * 256 + (btAryData[2] & 0xFF);
			end.mReadRate = (btAryData[3] & 0xFF) * 256 + (btAryData[4] & 0xFF);
			end.mTotalRead = (btAryData[5] & 0xFF) * 256 * 256 * 256 + (btAryData[6] & 0xFF) * 256 * 256
					+ (btAryData[7] & 0xFF) * 256 + (btAryData[8] & 0xFF);
			end.cmd = btCmd;
	...
	private void processGetAccessEpcMatch(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			if (btAryData[0] == 0x00) {
				m_curReaderSetting.mMatchEpcValue = StringTool.byteArrayToString(btAryData, 2, btAryData[1] & 0xFF);
				refreshSetting(m_curReaderSetting);
				onExeCMDStatus(btCmd, ERROR.SUCCESS);
				return;
			} else {
	...
	private void processRealTimeInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			end.mReadRate = (btAryData[1] & 0xFF) * 256 + (btAryData[2] & 0xFF);
			end.mTotalRead = (btAryData[3] & 0xFF) * 256 * 256 * 256 + (btAryData[4] & 0xFF) * 256 * 256
	...
	private void processFastSwitchInventory(MessageTran msgTran) {
		...
		if (btAryData.length == 0x01) {
			...
			int nSwitchTime = (btAryData[3] & 0xFF) * 255 * 255 * 255 + (btAryData[4] & 0xFF) * 255 * 255
					+ (btAryData[5] & 0xFF) * 255 + (btAryData[6] & 0xFF);
	...
	/**
	 * processCustomizedSessionTargetInventory and processRealTimeInventory return
	 * consistent.
	 *
	...
	/**
	 * processGetAndResetInventoryBuffer and processGetInventoryBuffer return
	 * consistent.
	 *
	 * @param msgTran Packet contents
	 */
	...
	private void processTagMask(MessageTran msgTran) {
		/*
		 * byte[] btAryData = msgTran.getAryData(); if (btAryData.length == 1) { if
		 * (btAryData[0] == (byte) 0x10) {
		 * onExeCMDStatus(msgTran.getCmd(),ERROR.SUCCESS); return; } else if
		 * (btAryData[0] == (byte) 0x41) {
		 * onExeCMDStatus(msgTran.getCmd(),btAryData[0]); } else {
		 * onExeCMDStatus(msgTran.getCmd(),ERROR.UNKONW_ERROR); } } else { if
		 * (btAryData.length > 7) { MaskMap map = new MaskMap(msgTran.getAryData()); if
		 * (!m_curReaderSetting.mMaskValue.contains(map)) {
		 * m_curReaderSetting.mMaskValue.add(map); mMaskQuantity++;
		 * //System.out.format("Maskquantity %d", mMaskQuantity); } if (mMaskQuantity ==
		 * map.mMaskQuantity) { mMaskQuantity = 0; refreshSetting(m_curReaderSetting); }
		 * } }
		 */
		if (msgTran != null) {
	...
	private String getFreqString(byte btFreq) {
		if (m_curReaderSetting.btRegion == 4) {
			float nExtraFrequency = (float) (btFreq & 0xFF) * (m_curReaderSetting.btUserDefineFrequencyInterval & 0xFF)
					* 10;
	...
	/**
	 * This method will callback when you send command to inquire the settings of
	 * reader.
	 * 
	 * @param readerSetting return the ReaderSetting instance it include the inquire
	 *                      parameters.
	 */
	...
	/**
	 * This method will callback when you send command to reader.Return the status
	 * of command.
	 * 
	...
	/**
	 * This method will callback when you send inventory command.Return inventoried
	 * tag it will be wrapped {@link RXInventoryTag} object.
	 * 
	...
	/**
	 * This method will callback when inventory end.Return
	 * {@link RXInventoryTag.RXInventoryTagEnd} contain end information.
	 * 
	...
	/**
	 * This method will callback when fast switch antenna inventory end.Return
	 * {@link RXInventoryTag.com.rfid.rxobserver.bean.RXInventoryTag.RXFastSwitchAntInventoryTagEnd}
	 * contain end information.
	 * 
	 * @param tagEnd {@link RXInventoryTag.com.rfid.rxobserver.bean.RXInventoryTag.RXFastSwitchAntInventoryTagEnd}
	 *               object.
	 */
	 ...
	 * 
	 * @param nAntID  current antenna id.
	 * @param nStatus Lock 6B return status (0x00:The byte is successfully locked.
	 *                0xFE:The byte is already locked. 0xFF:The byte can’t be
	 *                locked)
	 */
	 ...
	 * 
	 * @param nAntID  current antenna id.
	 * @param nStatus Lock 6B return status (0x00:The byte is not locked.. 0xFE:The
	 *                byte is locked.)
	 */
	...
	/**
	 * This method will callback when you send read,write,lock or kill
	 * command.Return information it will be wrapped {@link RXOperationTag}
	 * object.It contain the information you want to get.
	 */
	...
	/**
	 * The method will callback when execute the commands about tag mask include
	 * {@link com.rfid.RFIDReaderHelper#setTagMask(byte, byte, byte, byte, byte, byte, byte, byte[])}}
