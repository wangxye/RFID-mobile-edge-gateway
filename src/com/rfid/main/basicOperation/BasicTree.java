...
import com.rfid.main.util.GetSerialPort;
...
public class BasicTree {
	...
	public static void getDeviceIP(DefaultTreeModel model, JTree tree, DefaultMutableTreeNode[] nodeTree,
			DefaultMutableTreeNode node) {
		node.removeAllChildren();
		if (RFIDUtil.secondhandler
				.dllInit(System.getProperty("java.library.path").replace("\\", "/") + "/ZLDevManage")) {
			// get network IP
			int devCnt = RFIDUtil.secondhandler.startSearchDev();
			// String [] IPAddress = new String[devCnt];
			nodeTree = new DefaultMutableTreeNode[devCnt];
			for (byte i = 0; i < devCnt; ++i) {
				String IP = RFIDUtil.secondhandler.getIP(i).toString();
				// IPAddress[i] = IP;
	...
	@SuppressWarnings("rawtypes")
	public static void getDeviceComm(DefaultTreeModel model, JTree tree, DefaultMutableTreeNode[] nodeTree,
			DefaultMutableTreeNode node) {
		node.removeAllChildren();
